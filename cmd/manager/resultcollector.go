...
func defineResultcollectorFlags(cmd *cobra.Command) {
	...
	cmd.Flags().Bool("raw-results-output", true, "Setting to true to upload raw arf result")
	flags := cmd.Flags()
...
func parseConfig(cmd *cobra.Command) *scapresultsConfig {
	...
	conf.RawResultsOutput, _ = cmd.Flags().GetBool("raw-results-output")
...
func handleCompleteSCAPResults(exitcode string, scapresultsconf *scapresultsConfig, client *complianceCrClient) {
	...
	numWG := 1
	if scapresultsconf.RawResultsOutput {
		numWG++
	}
	wg.Add(numWG)
